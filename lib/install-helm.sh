...
helm_diff_version=${helm_diff_version:-latest}
helm_secrets_version=${helm_secrets_version:-latest}
...
krun::install::helm::install_helm_plugin() {
    local helm_bin="$1"
    local plugin_url="$2"
    local output verify_args=()

    mapfile -t verify_args < <(krun::install::helm::plugin_verify_args)

    if output=$("$helm_bin" plugin install "${verify_args[@]}" "$plugin_url" 2>&1); then
        echo "✓ Plugin installed: ${plugin_url}"
        return 0
    fi

    echo "Direct plugin install failed, trying proxy URL..." >&2
    if output=$("$helm_bin" plugin install "${verify_args[@]}" "https://ghproxy.link/${plugin_url}" 2>&1); then
        echo "✓ Plugin installed via proxy: ${plugin_url}"
        return 0
    fi

    echo "⚠ Plugin installation failed: ${plugin_url}" >&2
...
krun::install::helm::install_helm_diff() {
    local helm_bin="$1"
    local tag platform plugin_url output verify_args=()
    ...
    # Official install: repo URL + release tag (Helm downloads the platform tarball).
    echo "Installing helm-diff ${tag}..."
    ...
    # Fallback: direct release tarball (linux-amd64 / macos-arm64 naming).
    ...
    echo "Trying helm-diff tarball (${platform})..." >&2
    if krun::install::helm::install_helm_plugin "$helm_bin" "$plugin_url"; then
        return 0
    fi
...
krun::install::helm::install_helm_secrets() {
    local helm_bin="$1"
    local major tag version_plain plugin_url
    ...
    if [[ "$major" -ge 4 ]]; then
        ...
        local package plugin_base="https://github.com/jkroepke/helm-secrets/releases/download/${tag}"

        echo "Installing helm-secrets ${tag} (Helm 4 split packages)..."
        for package in "${packages[@]}"; do
            plugin_url="${plugin_base}/${package}"
            krun::install::helm::install_helm_plugin "$helm_bin" "$plugin_url" || true
        done
    ...
    echo "Installing helm-secrets ${tag} (Helm 3)..."
    local output verify_args=()
    mapfile -t verify_args < <(krun::install::helm::plugin_verify_args)

    if output=$("$helm_bin" plugin install "${verify_args[@]}" "https://github.com/jkroepke/helm-secrets" --version "$tag" 2>&1); then
        echo "✓ Plugin installed: helm-secrets ${tag}"
        return 0
    fi

    if output=$("$helm_bin" plugin install "${verify_args[@]}" "https://ghproxy.link/https://github.com/jkroepke/helm-secrets" --version "$tag" 2>&1); then
        echo "✓ Plugin installed via proxy: helm-secrets ${tag}"
        return 0
    fi

    plugin_url="https://github.com/jkroepke/helm-secrets/releases/download/${tag}/helm-secrets.tar.gz"
    krun::install::helm::install_helm_plugin "$helm_bin" "$plugin_url"
}
...
krun::install::helm::install_helm_git() {
    ...
    if ! command -v git >/dev/null 2>&1; then
        echo "⚠ git is required for helm-git; install git and retry" >&2
        return 1
    fi
