...
from handlers import config, install, ops
...
def _system_baseline(mode: str) -> None:
    require = __import__("krun_common").require_root
    require()
    if mode == "check":
...
"""
    ...
    from krun_common import write_if_changed, service_enable
...
SCRIPTS: dict[str, callable] = {
    ...
    "install_vim": lambda: __import__("krun_common").install_packages(deb=["vim"], rhel=["vim-enhanced"], brew=["vim"]),
    "install_ruby": lambda: __import__("krun_common").install_packages(deb=["ruby-full"], rhel=["ruby"], brew=["ruby"]),
    "install_tinyproxy": lambda: __import__("krun_common").install_packages(
    ...
    "install_jdk8": lambda: __import__("krun_common").install_packages(
    ...
    "install_filebeat": lambda: __import__("krun_common").install_packages(deb=["filebeat"], rhel=["filebeat"], epel=True),
    "install_vagrant_virtualbox": lambda: __import__("krun_common").install_packages(
    ...
    "config_cursor": lambda: __import__("krun_common").run(
    ...
    "update_vagrant_box": lambda: __import__("krun_common").run(["vagrant", "box", "update", "--all"]),
}
