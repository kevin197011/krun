#!/usr/bin/env python3
"""Browser-like HTTP fetch helpers for krun (urllib + curl)."""
...
KRUN_ID = "krun/2.1"
USER_AGENT = f"{BROWSER_CORE} {KRUN_ID}"

RETRYABLE_HTTP = {429, 502, 503, 504}


def browser_headers(url: str) -> dict[str, str]:
    """Headers that resemble a normal browser navigation/download."""
    parsed = urlparse(url)
...
def mirror_urls(url: str) -> list[str]:
    """CDN/mirror first — raw GitHub rate-limits aggressively (HTTP 429)."""
    urls: list[str] = []
    mirror = os.environ.get("KRUN_PY_MIRROR", "").strip()
    if mirror:
        urls.append(f"{mirror.rstrip('/')}/{url}")
...
def _retry_delay(attempt: int) -> None:
    # ponytail: jitter so retries do not look like a tight bot loop
        ...
        for attempt in range(2):
            try:
                ...
                if exc.code == 429:
                    ...
                    break
                if exc.code in RETRYABLE_HTTP and attempt < 1:
...
def curl_headers(url: str) -> list[str]:
    args: list[str] = []
    for key, value in browser_headers(url).items():
...
def curl_cmd(
    ...
    *,
    ...
    cmd = ["curl", "-fsSL", *curl_headers(url)]
    if head:
        cmd.insert(1, "-I")
